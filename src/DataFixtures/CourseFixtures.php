...
class CourseFixtures extends Fixture {
    public function load(ObjectManager $manager)
    {
        ...
        $lesson11->setContent("Данный урок посвящен показательной функции. Мы введем определение, покажем необходимость и актуальность данной функции. Далее охарактеризуем ее свойства и поведение в общем и частных случаях, построим графики и сравним их.");
        $lesson11->setNumber(1);

        $lesson12 = new Lesson();
        $lesson12->setCourse($course1);
        $lesson12->setTitle("Тригонометрические уравнения, вычисления");
        $lesson12->setContent("На этом уроке мы повторим тригонометрические уравнения и вычисления.
        ...
        $course2->setDescription("Курс языка Python");

        $lesson21 = new Lesson();
        $lesson21->setCourse($course2);
        $lesson21->setTitle("Общая информация о курсе");
        $lesson21->setContent("Данный урок предоставляет информацию о курсе, какиец цели ставили перед собой его создатели, какие навыки получат прошедшие курс люди");
        $lesson21->setNumber(1);

        $lesson22 = new Lesson();
        $lesson22->setCourse($course2);
        $lesson22->setTitle("Введение: программы и Python. Проверка заданий");
        $lesson22->setContent("Какое место занимает Python в современном мире? Пример самых популярных программ на этом языке, сфера его применения.");
        $lesson22->setNumber(2);

        $lesson23 = new Lesson();
        $lesson23->setCourse($course2);
        $lesson23->setTitle("Интерактивный режим Python. IPython");
        $lesson23->setContent("Познакомимся с интерактивных режимом Python, узнаем какие возможности предоставляет этот режим разработчику. Итак, чтобы войти в интерактивный режим Python...");
        $lesson23->setNumber(3);

        $lesson24 = new Lesson();
        $lesson24->setCourse($course2);
        $lesson24->setTitle("Операции с целыми числами");
        $lesson24->setContent("Операции с целыми числами в Python проводятся с операторами + и -, умножение и деление приводит числа в тип с плавающей точкой...");
        $lesson24->setNumber(4);


        # fixture for 3rd course with lessons
        $course3 = new Course();
        $course3->setSymbolicCode("01C");
        ...
        $lesson31->setContent("научитесь использовать главный инструмент для работы с интерфейсами. узнаете как проектировать структуру сайтов, создавать прототипы страниц и работать над улучшением ux");
        $lesson31->setNumber(1);

        $lesson32 = new Lesson();
        $lesson32->setCourse($course3);
        $lesson32->setTitle("основы дизайна");
        $lesson32->setContent("познакомитесь с ключевыми понятиями и принципами дизайна, лежащими в основе любого изображения и макета. выполните практические задания по разработке сайтов и рекламной графики для закрепления материала");
        $lesson32->setNumber(2);

        $lesson33 = new Lesson();
        $lesson33->setCourse($course3);
        $lesson33->setTitle("blender");
        $lesson33->setContent("освоите навыки работы с 3d, научитесь быстро подбирать референсы и уже готовые модели, чтобы добавлять эффектную графику на сайты");
        $lesson33->setNumber(3);


        # fixture for 4th course with lessons 
        $course4 = new Course();
        $course4->setSymbolicCode("01D");
        ...
        $lesson41->setContent("Познакомитесь с популярными социальными сетями в России и поймёте их алгоритмы работы. Изучите роли и задачи SMM в бизнесе и узнаете, какие навыки должны быть у SMM-менеджера.");
        $lesson41->setNumber(1);

        $lesson42 = new Lesson();
        $lesson42->setCourse($course4);
        $lesson42->setTitle("Погружаемся в стратегию");
        $lesson42->setContent("Поймёте роль SMM-стратегии в бизнесе. Научитесь брифовать заказчиков, анализировать конкурентов и целевую аудиторию, составлять портреты целевой аудитории и говорить с ней на одном языке. Разберётесь, что такое KPI и как их правильно определять. Сможете решать тактические задачи бренда.");
        $lesson42->setNumber(2);

        $lesson43 = new Lesson();
        $lesson43->setCourse($course4);
        $lesson43->setTitle("Контент для социальных сетей");
        $lesson43->setContent("Познакомитесь с основными типами контента и поймёте их роль в маркетинге. Узнаете, как формировать tone of voice. Научитесь искать темы для постов, составлять рубрикатор и контент-план.");
        $lesson43->setNumber(3);
